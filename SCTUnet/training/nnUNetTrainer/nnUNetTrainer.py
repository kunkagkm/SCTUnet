...
from nnunetv2.training.loss.compound_losses import DC_and_CE_loss, DC_and_BCE_loss, DC_and_Focal_and_BD_loss
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.print_to_log_file("\n#######################################################################\n"
                               "Please cite the following paper when using nnU-Netg2:\n"
    ...
    def _build_loss(self):
        if self.label_manager.has_regions:
            ...
            loss = DC_and_Focal_and_BD_loss( {'batch_dice': self.configuration_manager.batch_dice,
                                               'do_bg': True, 'smooth': 1e-5, 'ddp': self.is_ddp}, {'alpha':0.5, 'gamma':0, 'smooth':1e-5}, {})            
            print("Using DC_and_focal_and_BD_loss")          
    ...
    def train_step(self, batch: dict) -> dict:
        data = batch['data']
        target = batch['target']
        current_epoch_list = self.current_epoch  # 假设这是一个整数
        current_epoch = [current_epoch_list]  # 创建一个只有一个元素的列表
